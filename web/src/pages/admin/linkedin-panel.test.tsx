...
const paint = (status: LinkedInStatus, props: Partial<Parameters<typeof LinkedInPanel>[0]> = {}) => {
  ...
  render(<LinkedInPanel config={config()} api={client} busy={false}
...
describe('LinkedInPanel', () => {
  ...
  it('"compartilhar agora" com a fila vazia explica em vez de mentir que publicou', async () => {
    ...
    const client = api({ connected: true, hasApp: true, daysLeft: 50 })
    render(<LinkedInPanel config={config()} api={client} busy={false} onSave={vi.fn()} onMessage={onMessage} />)
