...
from . import gemini, images, model, store
...
def generate(topic: str | None, now: datetime | None = None, context: str = "") -> dict[str, Any]:
    ...
    if not (topic or "").strip():
        ...
        if not topic:
            raise NoTopicError("a pauta acabou — cadastre novos temas no painel")

    draft = gemini.generate_post(topic, context=context)
    draft["topic"] = topic
    draft["generation"] = {"model": draft.get("model", ""), "generatedAt": now, "topic": topic}
